...
int main(void) {
    ...
    EVAL:
        ...
        if (isNum(expr)) goto NUMBER;

        if (isVar(expr)) goto VARIABLE;

        if (isIf(expr)) goto IF;

        if (isLambda(expr)) goto LAMBDA;

        if (isDef(expr)) goto DEFINITION;

        if (isAss(expr)) goto ASSIGNMENT;

        if (isAnd(expr)) goto AND;

        if (isOr(expr)) goto OR;

        if (isBegin(expr)) goto BEGIN;

        if (isQuote(expr)) goto QUOTATION;

        if (isDelay(expr)) goto DELAY;
    ...
    DEFINITION:
        ...
        if (isSugarDef(expr)) expr = transformSugarDef(expr);
    ...
    FUNCTION:
        ...
        if (isSimple(expr)) goto SIMPLE_FUNC;
    ...
    SIMPLE_FUNC:
        ...
        if (isUnbound(func)) goto UNBOUND;
    ...
    CHECK_NO_ARGS:
        ...
        if (noCompoundArgs(unev)) goto NO_COMPOUND_ARGS;
    ...
    ARG_LOOP:
        ...
        if (isSimple(expr)) goto SIMPLE_ARG;
    ...
    COMPOUND_ARG:
        ...
        // avoids two saves
        if (isLastArg(unev)) goto LAST_ARG;
    ...
    DID_SIMPLE_ARG:
        ...
        if (isLastArg(unev)) goto RESTORE_FUNC;
    ...
    APPLY:
        ...
        if (isPrimitive(func)) goto APPLY_PRIMITIVE;

        if (isCompiled(func)) goto APPLY_COMPILED;

        if (isCompound(func)) goto APPLY_COMPOUND;
    ...
    SEQUENCE: // SEQUENCE never receives an empty list
        ...
        if (isLastExp(unev)) goto LAST_EXP;
    ...
    CONTINUE:
        ...
        // interpreter labels
        if (GETLABEL(cont) == _DONE) goto DONE;

        if (GETLABEL(cont) == _IF_DECIDE) goto IF_DECIDE;

        if (GETLABEL(cont) == _DID_ASS_VAL) goto DID_ASS_VAL;

        if (GETLABEL(cont) == _DID_DEF_VAL) goto DID_DEF_VAL;

        if (GETLABEL(cont) == _DID_FUNC) goto DID_FUNC;

        if (GETLABEL(cont) == _ACC_ARG) goto ACC_ARG;

        if (GETLABEL(cont) == _DID_LAST_ARG) goto DID_LAST_ARG;

        if (GETLABEL(cont) == _DID_SIMPLE_ARG) goto DID_SIMPLE_ARG;

        if (GETLABEL(cont) == _REST_SIMPLE_ARGS) goto REST_SIMPLE_ARGS;

        if (GETLABEL(cont) == _SEQ_CONT) goto SEQ_CONT;

        if (GETLABEL(cont) == _ALT_SEQ_CONT) goto ALT_SEQ_CONT;
